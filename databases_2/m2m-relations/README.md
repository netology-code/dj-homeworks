...
Есть небольшой новостной сайт. Было решено к статьям добавить тематические резделы
...
Для реализации на странице редактирования объекта
возможности редактировать связязи через таблицу многие ко многим
в админке использовать свойство `inlines`:
...
а вместо `Relationship` должны быть модель связи указанная как `though` для связи.
